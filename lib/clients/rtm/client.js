...
RTMClient.prototype._onStart = function _onStart(err, data) {
  ...
  if (err || !data.url) {
    this.logger.log('verbose', 'failed to connect to the RTM API: ' + err, data.error);
    this.emit(CLIENT_EVENTS.FAILED_AUTHENTICATION, err || data);
