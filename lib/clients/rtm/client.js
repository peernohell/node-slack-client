...
RTMClient.prototype.reconnect = function reconnect() {
  if (!this._reconnecting) {
    ...
    setTimeout(bind(this.start, this), (this._connAttempts - 1) * this.RECONNECTION_BACKOFF);
  }
};
...
RTMClient.prototype._pingServer = function _pingServer() {
  ...
  if (this.connected) {
    ...
    if (pongInterval > this.MAX_PONG_INTERVAL) {
      this.logger('info', 'Pong reconnect', this.activeTeamId, this.ws.readyState, pongInterval, Date.now(), this._lastPong);
      this.reconnect();
    } else {
      this.logger('verbose', 'Ping server', this.activeTeamId, this.ws.readyState, Date.now());
...
RTMClient.prototype._handleWsMessageInternal = function _handleWsMessageInternal(
  ...
  if (messageType === 'pong') {
    ...
    this.logger('info', 'TEAM_MIGRATION_STARTED reconnect', this.activeTeamId, this.ws.readyState);
    this.reconnect();
...
RTMClient.prototype.handleWsClose = function handleWsClose(code, reason) {
  ...
  if (this.autoReconnect) {
    if (!this._connecting) {
      this.logger('info', 'WS_CLOSE reconnect', this.activeTeamId, this.ws.readyState);
      this.reconnect();
    }
  } else {
...
RTMClient.prototype._handlePong = function _handlePong(message) {
  this._lastPong = Date.now();
  this.logger('verbose', 'Pong recived', this.activeTeamId, this.ws.readyState, Date.now());
