...
var handleLeave = function handleLeave(activeUserId, activeTeamId, dataStore, message) {
  ...
  var baseChannel = dataStore.getChannelGroupOrDMById(message.channel);
  if (baseChannel) {
    index = baseChannel.members.indexOf(message.user || activeUserId);
    if (index !== -1) {
 ...
 */
var handleJoin = function handleJoin(dataStore, message) {
  var index;
  var baseChannel = dataStore.getChannelGroupOrDMById(message.channel);
  if (baseChannel) {
    index = baseChannel.members.indexOf(message.user);
    if (index === -1) {
